...
export class AsignacionesMaterialesService {
  ...
  async create(createDto: CreateAsignacionMaterialDto, usuarioId: number) {
    ...
    if (tarea.etapa.proyecto.estado === 'finalizado' || tarea.etapa.proyecto.estado === 'cancelado') {
      throw new ConflictException(`No se pueden asignar materiales en un proyecto ${tarea.etapa.proyecto.estado}`);
    }
    ...
    if (existingAsignacion) {
      throw new ConflictException(`Ya existe una asignación para este material en esta tarea`);
    }
  ...
  async update(id: number, updateDto: UpdateAsignacionMaterialDto, usuarioId: number) {
    ...
    if (asignacion.tarea.etapa.proyecto.estado === 'finalizado' || asignacion.tarea.etapa.proyecto.estado === 'cancelado') {
      throw new ConflictException(`No se pueden actualizar asignaciones en un proyecto ${asignacion.tarea.etapa.proyecto.estado}`);
    }
