...
export class UsersService {
  ...
  async create(data: CreateUserDto, usuarioCreadorId?: number): Promise<Usuario> {
  ...
  async update(id: number, data: UpdateUserDto, usuarioModificadorId: number): Promise<Usuario> {
    ...
    const usuarioActualizado = await this.prisma.$transaction(async (prisma) => {
      ...
      if (rolesToConnect.length > 0) {
        await prisma.usuario.update({
          ...
          data: {
            roles: {
              disconnect: usuarioExistente.roles.map(role => ({ id: role.id })),
            },
          },
        });
      }
      ...
      return prisma.usuario.update({
        ...
        data: {
          ...updateData,
          ...(rolesToConnect.length > 0 ? { roles: { connect: rolesToConnect } } : {}),
